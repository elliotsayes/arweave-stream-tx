...
function uploadTransactionAsync(tx, arweave, createTx = true) {
    return async (source) => {
        ...
        await promises_1.pipeline(source, stream_chunker_1.default(merkle_1.MAX_CHUNK_SIZE, { flush: true }), async function (chunkedSource) {
            var e_1, _a;
            let chunkIndex = 0;
            ...
            try {
                for (var chunkedSource_1 = __asyncValues(chunkedSource), chunkedSource_1_1; chunkedSource_1_1 = await chunkedSource_1.next(), !chunkedSource_1_1.done;) {
                    const volatileChunkData = chunkedSource_1_1.value;
                    async function processChunkData(chunkData) {
                        if (chunkData.byteLength > merkle_1.MAX_CHUNK_SIZE) {
                            // This can happen when the read stream is flushed
                            // In this case, process the head chunk and recurse through the tail
                            const chunkHead = chunkData.slice(0, merkle_1.MAX_CHUNK_SIZE);
                            await processChunkData(chunkHead);
                            const chunkTail = chunkData.slice(merkle_1.MAX_CHUNK_SIZE);
                            await processChunkData(chunkTail);
                    ...
                    processChunkData(volatileChunkData);
                }
            }
